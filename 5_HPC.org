...
$ sudo ./result-4/bin/pixiecore boot vmlinuz64 corepure64.gz rootfs64.gz modules64.gz --dhcp-no-bind -d -p80

$ ./result-3/bin/wakeonlan -p 8 54:05:DB:DC:4C:9D
...
| * generate DOS file system    | # mkdosfs /dev/sdc1                                        | [[https://www.debian.org/releases/etch/i386/ch04s04.html.ko][link]] |
| * install ldlinux(bootloader) | # ./result-5/bin/syslinux /dev/sdc1                        |      |
| * usb mount                   | # mount /dev/sdc1 /mnt                                     |      |
| * kernel initrd files copy    | # cp vmlinuz64 corepure64.gz rootfs64.gz modules64.gz /mnt |      |
| * generate syslinux.cfg       | # cat > syslinux.cfg                                       |      |
| syslinux.cfg contents         | default vmlinuz                                            |      |
|                               | append initrd=initrd.gz                                    |      |
|                               |                                                            |      |
| * usb(sdc1) unmount           | # umount /mnt                                              |      |
|                               |                                                            |      |
